...
def calc_save_device_lstm2_in_feature_names(DAG, device_names):
    ...
    device_lstm2_inout_feature_names = defaultdict(lambda: {'in': []})
    for d_name in device_names:
        in_feature_names = calc_lstm2_in_feature_names(DAG, d_name)
        device_lstm2_inout_feature_names[d_name]['in'] = in_feature_names
    with open('data-dc/device_lstm2_in_feature_names.json', 'w') as f:
        json.dump(device_lstm2_inout_feature_names, f, sort_keys=True, indent=2)
    return device_lstm2_inout_feature_names


def load_device_lstm2_inout_feature_names():
    with open('data-dc/device_lstm2_in_feature_names.json', 'r') as f:
        device_lstm_inout_feature_names = json.load(f)
    return device_lstm_inout_feature_names


def calc_lstm2_in_feature_names(DAG, device_name):
